...
static uint8_t r100_in(uint16_t addr, void *priv);
static void    r100_out(uint16_t addr, uint8_t val, void *priv);

static uint8_t
r100_pci_read(UNUSED(int func), int addr, void *priv)
{
    const r100_t *r100 = (r100_t *) priv;
#if 0
...
static void
r100_pci_write(UNUSED(int func), int addr, uint8_t val, void *priv)
{
    r100_t *r100 = (r100_t *) priv;
...
uint32_t
r100_mmio_read_l(uint32_t addr, void *priv)
{
    r100_t       *r100 = (r100_t *) priv;
    const svga_t *svga = &r100->svga;
...
uint8_t
r100_mmio_read(uint32_t addr, void *priv)
{
    r100_t *r100 = (r100_t *) priv;
...
uint16_t
r100_mmio_read_w(uint32_t addr, void *priv)
{
    r100_t *r100 = (r100_t *) priv;
...
void
r100_mmio_write_l(uint32_t addr, uint32_t val, void *priv)
{
    r100_t *r100 = (r100_t *) priv;
...
void
r100_mmio_write(uint32_t addr, uint8_t val, void *priv)
{
    uint32_t tmp;
    ...
    if ((addr >= 0xf00) && (addr <= 0xfff)) {
        r100_pci_write(0, addr & 0xff, val, priv);
        return;
    }

    tmp = r100_mmio_read_l(addr, priv);
    tmp &= ~(0xff << ((addr & 3) << 3));
    tmp |= val << ((addr & 3) << 3);
    r100_mmio_write_l(addr, tmp, priv);
}

void
r100_mmio_write_w(uint32_t addr, uint16_t val, void *priv)
{
    uint32_t tmp;
    ...
    if ((addr >= 0xf00) && (addr <= 0xfff)) {
        r100_pci_write(0, addr & 0xff, val, priv);
        r100_pci_write(0, (addr + 1) & 0xff, val, priv);
        return;
    }
...
static void
r100_out(uint16_t addr, uint8_t val, void *priv)
{
    r100_t *r100 = (r100_t *) priv;
    svga_t *svga = &r100->svga;
    uint8_t old;
...
static uint8_t
r100_in(uint16_t addr, void *priv)
{
    r100_t *r100 = (r100_t *) priv;
    svga_t *svga = &r100->svga;
    uint8_t temp;
...
static void
r100_recalctimings(UNUSED(svga_t *svga))
{
    // TODO
}
...
r100_init(const device_t *info)
{
    ...
    svga_t     *svga;
    const char *romfn = BIOS_R100_PATH;
