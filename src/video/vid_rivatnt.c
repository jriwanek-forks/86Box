...
typedef struct rivatnt_t {
    ...
    struct {
        ...
        struct {
            uint32_t push_enabled;
            uint32_t pull_enabled;
            uint32_t status0;
            uint32_t status1;
            uint32_t put;
            uint32_t get;
...
static uint8_t rivatnt_in(uint16_t addr, void *priv);
static void    rivatnt_out(uint16_t addr, uint8_t val, void *priv);

static uint8_t
rivatnt_pci_read(UNUSED(int func), int addr, void *priv)
{
    const rivatnt_t *rivatnt = (rivatnt_t *) priv;
#if 0
    ...
    switch (addr) {
        ...
        case 0x04:
            return rivatnt->pci_regs[0x04] & 0x37; /*Respond to IO and memory accesses*/
        case 0x05:
...
static void
rivatnt_pci_write(UNUSED(int func), int addr, uint8_t val, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
...
uint32_t
rivatnt_pmc_recompute_intr(void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
    uint32_t   intr    = 0;
    ...
    else if ((intr & (1 << 31)) && (rivatnt->pmc.intr_en & 2))
        pci_set_irq(rivatnt->pci_slot, PCI_INTA, &rivatnt->irq_state);
#if 0
    else
...
uint32_t
rivatnt_pmc_read(uint32_t addr, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
...
void
rivatnt_pmc_write(uint32_t addr, uint32_t val, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
...
uint32_t
rivatnt_pfifo_read(uint32_t addr, void *priv)
{
    const rivatnt_t *rivatnt = (rivatnt_t *) priv;
...
void
rivatnt_pfifo_write(uint32_t addr, uint32_t val, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
...
void
rivatnt_ptimer_interrupt(int num, void *priv)
{
#if 0
...
uint32_t
rivatnt_ptimer_read(uint32_t addr, void *priv)
{
    const rivatnt_t *rivatnt = (rivatnt_t *) priv;
...
void
rivatnt_ptimer_write(uint32_t addr, uint32_t val, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
...
uint32_t
rivatnt_pfb_read(uint32_t addr, void *priv)
{
    const rivatnt_t *rivatnt = (rivatnt_t *) priv;
...
uint32_t
rivatnt_pextdev_read(uint32_t addr, UNUSED(void *priv))
{
#if 0
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
#endif
...
uint32_t
rivatnt_pcrtc_read(uint32_t addr, void *priv)
{
    const rivatnt_t *rivatnt = (rivatnt_t *) priv;
...
void
rivatnt_pcrtc_write(uint32_t addr, uint32_t val, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
    switch (addr) {
        case 0x600100:
...
uint32_t
rivatnt_pramdac_read(uint32_t addr, void *priv)
{
    const rivatnt_t *rivatnt = (rivatnt_t *) priv;
...
void
rivatnt_pramdac_write(uint32_t addr, uint32_t val, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
    switch (addr) {
        case 0x680500:
...
#if 0
    if (cs == 0x0008 && !rivatnt->pgraph.beta)
        nv_riva_log("RIVA TNT PTIMER time elapsed %f alarm %08x, time_low %08x\n", time, rivatnt->ptimer.alarm, rivatnt->ptimer.time & 0xffffffff);
#endif
...
uint32_t
rivatnt_mmio_read_l(uint32_t addr, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
    ...
    switch (addr) {
        ...
        case 0x6813ca:
        case 0x6813cb:
            ret = (rivatnt_in((addr + 0) & 0x3ff, priv) << 0) | (rivatnt_in((addr + 1) & 0x3ff, priv) << 8) | (rivatnt_in((addr + 2) & 0x3ff, priv) << 16) | (rivatnt_in((addr + 3) & 0x3ff, priv) << 24);
            break;
    ...
    if ((addr >= 0x700000) && (addr <= 0x7fffff))
        ret = rivatnt->ramin[(addr & 0xfffff) >> 2];
    if ((addr >= 0x300000) && (addr <= 0x30ffff))
        ret = ((uint32_t *) rivatnt->bios_rom.rom)[(addr & rivatnt->bios_rom.mask) >> 2];

    if ((addr >= 0x1800) && (addr <= 0x18ff))
        ret = (rivatnt_pci_read(0, (addr + 0) & 0xff, priv) << 0) | (rivatnt_pci_read(0, (addr + 1) & 0xff, priv) << 8) | (rivatnt_pci_read(0, (addr + 2) & 0xff, priv) << 16) | (rivatnt_pci_read(0, (addr + 3) & 0xff, priv) << 24);
...
uint8_t
rivatnt_mmio_read(uint32_t addr, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
    ...
    if ((addr >= 0x1800) && (addr <= 0x18ff))
        return rivatnt_pci_read(0, addr & 0xff, priv);
    ...
    switch (addr) {
        ...
        case 0x6813ca:
        case 0x6813cb:
            return rivatnt_in(addr & 0x3ff, priv);
...
uint16_t
rivatnt_mmio_read_w(uint32_t addr, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
    ...
    if ((addr >= 0x300000) && (addr <= 0x30ffff))
        return ((uint16_t *) rivatnt->bios_rom.rom)[(addr & rivatnt->bios_rom.mask) >> 1];

    if ((addr >= 0x1800) && (addr <= 0x18ff))
        return (rivatnt_pci_read(0, (addr + 0) & 0xff, priv) << 0) | (rivatnt_pci_read(0, (addr + 1) & 0xff, priv) << 8);
    ...
    switch (addr) {
        ...
        case 0x6813ca:
        case 0x6813cb:
            return (rivatnt_in((addr + 0) & 0x3ff, priv) << 0) | (rivatnt_in((addr + 1) & 0x3ff, priv) << 8);
...
void
rivatnt_mmio_write_l(uint32_t addr, uint32_t val, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
    ...
    if ((addr >= 0x1800) && (addr <= 0x18ff)) {
        rivatnt_pci_write(0, addr & 0xff, val & 0xff, priv);
        rivatnt_pci_write(0, (addr + 1) & 0xff, (val >> 8) & 0xff, priv);
        rivatnt_pci_write(0, (addr + 2) & 0xff, (val >> 16) & 0xff, priv);
        rivatnt_pci_write(0, (addr + 3) & 0xff, (val >> 24) & 0xff, priv);
        return;
    }
    ...
    if ((addr >= 0x700000) && (addr <= 0x7fffff))
        rivatnt->ramin[(addr & 0xfffff) >> 2] = val;
    ...
    switch (addr) {
        ...
        case 0x6813ca:
        case 0x6813cb:
            rivatnt_out(addr & 0xfff, val & 0xff, priv);
            rivatnt_out((addr + 1) & 0xfff, (val >> 8) & 0xff, priv);
            rivatnt_out((addr + 2) & 0xfff, (val >> 16) & 0xff, priv);
            rivatnt_out((addr + 3) & 0xfff, (val >> 24) & 0xff, priv);
            break;
...
void
rivatnt_mmio_write(uint32_t addr, uint8_t val, void *priv)
{
    uint32_t tmp;
    ...
    tmp = rivatnt_mmio_read_l(addr, priv);
    tmp &= ~(0xff << ((addr & 3) << 3));
    tmp |= val << ((addr & 3) << 3);
    rivatnt_mmio_write_l(addr, tmp, priv);
    if ((addr >= 0x1800) && (addr <= 0x18ff))
        rivatnt_pci_write(0, addr & 0xff, val, priv);
}

void
rivatnt_mmio_write_w(uint32_t addr, uint16_t val, void *priv)
{
    uint32_t tmp;
    ...
    switch (addr) {
        ...
        case 0x6813cb:
            rivatnt_out(addr & 0xfff, val & 0xff, priv);
            rivatnt_out(addr & 0xfff, val >> 8, priv);
            return;
...
uint8_t
rivatnt_rma_in(uint16_t addr, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
    svga_t    *svga    = &rivatnt->svga;
    uint8_t    ret     = 0;
...
void
rivatnt_rma_out(uint16_t addr, uint8_t val, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
    svga_t    *svga    = &rivatnt->svga;
...
static void
rivatnt_out(uint16_t addr, uint8_t val, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
    svga_t    *svga    = &rivatnt->svga;
    uint8_t    old;
            ...
            if (svga->seqregs[0x06] == 0x57) {
                switch (svga->crtcreg) {
                    case 0x1e:
                        ...
                        if (svga->chain4)
                            svga->read_bank = rivatnt->read_bank << 15;
                        else
                            svga->read_bank = rivatnt->read_bank << 13;
                        break;
                    case 0x1d:
                        rivatnt->write_bank = val;
                        if (svga->chain4)
                            svga->write_bank = rivatnt->write_bank << 15;
                        else
                            svga->write_bank = rivatnt->write_bank << 13;
                        break;
                    case 0x19:
                    case 0x1a:
                    case 0x25:
...
static uint8_t
rivatnt_in(uint16_t addr, void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
    svga_t    *svga    = &rivatnt->svga;
    uint8_t    temp;
...
rivatnt_init(const device_t *info)
{
    ...
    svga_t     *svga;
    const char *romfn = BIOS_RIVATNT_PATH;
...
void
rivatnt_speed_changed(void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
...
void
rivatnt_force_redraw(void *priv)
{
    rivatnt_t *rivatnt = (rivatnt_t *) priv;
