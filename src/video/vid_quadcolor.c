...
quadcolor_poll(void *priv)
{
    ...
    if (!quadcolor->linepos) {
        ...
        if (quadcolor->sc == (quadcolor->crtc[11] & 31) || ((quadcolor->crtc[8] & 3) == 3 &&
            quadcolor->sc == ((quadcolor->crtc[11] & 31) >> 1))) {
            quadcolor->con  = 0;
            quadcolor->coff = 1;
        }
        ...
        if (quadcolor->cgadispon)
            quadcolor->cgastat &= ~1;
        if (quadcolor->sc == (quadcolor->crtc[10] & 31) || ((quadcolor->crtc[8] & 3) == 3 &&
            quadcolor->sc == ((quadcolor->crtc[10] & 31) >> 1)))
            quadcolor->con = 1;
...
quadcolor_standalone_init(UNUSED(const device_t *info))
{
    ...
    timer_add(&quadcolor->timer, quadcolor_poll, quadcolor, 1);
    mem_mapping_add(&quadcolor->mapping, 0xb8000, 0x08000, quadcolor_read, NULL, NULL, quadcolor_write, NULL, NULL, NULL /*quadcolor->vram*/, MEM_MAPPING_EXTERNAL,
            quadcolor);
