...
static void
tandy_recalc_address(tandy_t *dev)
{
    t1kvid_t *vid = dev->vid;
...
static void
tandy_vid_write(uint32_t addr, uint8_t val, void *priv)
{
    tandy_t  *dev = (tandy_t *) priv;
    t1kvid_t *vid = dev->vid;
...
static uint8_t
tandy_vid_read(uint32_t addr, void *priv)
{
    const tandy_t  *dev = (tandy_t *) priv;
    const t1kvid_t *vid = dev->vid;
...
tandy_vid_init(tandy_t *dev)
{
    ...
    mem_mapping_add(&vid->mapping, 0xb8000, 0x08000,
                    tandy_vid_read, NULL, NULL, tandy_vid_write, NULL, NULL, NULL, 0, dev);
    io_sethandler(0x03d0, 16,
                  tandy_vid_in, NULL, NULL, tandy_vid_out, NULL, NULL, dev);
}

const device_config_t tandy_vid_config[] = {
  // clang-format off
    {
        .name = "display_type",
...
const device_t tandy_vid_device = {
    .name          = "Tandy 1000",
    ...
    .force_redraw  = NULL,
    .config        = tandy_vid_config
};

const device_t tandy_vid_device_hx = {
    .name          = "Tandy 1000 HX",
...
const device_t tandy_vid_device_sl = {
    .name          = "Tandy 1000SL2",
