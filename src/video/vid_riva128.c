...
uint8_t
riva128_ramin_read(uint32_t addr, void *priv)
{
    const riva128_t *riva128 = (riva128_t *) priv;
    const svga_t    *svga    = &riva128->svga;
...
riva128_pmc_recompute_intr(int send_intr, void *priv)
{
    ...
    if (riva128->pfifo.intr & riva128->pfifo.intr_en)
        intr |= (1 << 8);
    if ((riva128->pgraph.intr_0 & (1 << 8)) && (riva128->pgraph.intr_en_0 & (1 << 8)))
        intr |= (1 << 24);
    if ((riva128->pgraph.intr_0 & ~(1 << 8)) && (riva128->pgraph.intr_en_0 & ~(1 << 8)))
        intr |= (1 << 12);
...
#if 0
    else pci_clear_irq(riva128->pci_slot, PCI_INTA, &riva128->irq_state);
#endif
...
uint32_t
riva128_pmc_read(uint32_t addr, void *priv)
{
    riva128_t *riva128 = (riva128_t *) priv;
...
void
riva128_pmc_write(uint32_t addr, uint32_t val, void *priv)
{
    riva128_t *riva128 = (riva128_t *) priv;
    ...
    case 0x000100:
        ...
        if((val & (1u << 31)) && (riva128->pmc.intr_en & 2))
            pci_set_irq(riva128->pci_slot, PCI_INTA, &riva128->irq_state); /* Software interrupt */
#if 0
        else
            pci_clear_irq(riva128->pci_slot, PCI_INTA, &riva128->irq_state);
#endif
...
void
riva128_pfifo_interrupt(int num, void *priv)
{
    riva128_t *riva128 = (riva128_t *) priv;
...
uint32_t
riva128_pfifo_read(uint32_t addr, void *priv)
{
    riva128_t *riva128 = (riva128_t *) priv;
...
void
riva128_pfifo_write(uint32_t addr, uint32_t val, void *priv)
{
    riva128_t *riva128 = (riva128_t *) priv;
...
void
riva128_ptimer_interrupt(int num, void *priv)
{
#if 0
...
uint32_t
riva128_ptimer_read(uint32_t addr, void *priv)
{
    const riva128_t *riva128 = (riva128_t *) priv;
...
void
riva128_ptimer_write(uint32_t addr, uint32_t val, void *priv)
{
    riva128_t *riva128 = (riva128_t *) priv;
...
void
riva128_pfb_write(uint32_t addr, uint32_t val, void *priv)
{
    riva128_t *riva128 = (riva128_t *) priv;
...
void
riva128_pgraph_interrupt(int num, void *priv)
{
    riva128_t *riva128 = (riva128_t *) priv;
...
riva128_pgraph_color_t
riva128_pgraph_expand_color(uint32_t graphobj0, uint32_t color, UNUSED(void *priv))
{
#if 0
    riva128_t *riva128 = (riva128_t *) priv;
#endif
            ...
            switch(method)
            {
                ...
                case 0x404:
                {
                    ...
                    if (riva128->pgraph.lin_start_x == riva128->pgraph.lin_end_x) {
                        ...
                        pclog("RIVA 128 not a vertical or horizontal lin\n");
                    break;
...
void
riva128_do_cache0_puller(void *priv)
{
    riva128_t *riva128 = (riva128_t *) priv;
...
riva128_do_gpu_work(void *priv)
{
    ...
    if (riva128->pfifo.caches[1].dma_ctrl & 1) {
        ...
        for (int i = riva128->pfifo.caches[1].dma_addr;
                i <= (riva128->pfifo.caches[1].dma_addr + riva128->pfifo.caches[1].dma_length);
                i+=4) {
...
uint32_t
riva128_user_read(uint32_t addr, void *priv)
{
    riva128_t *riva128 = (riva128_t *) priv;
#if 0
...
riva128_mmio_read_l(uint32_t addr, void *priv)
{
    ...
    if ((addr >= 0x110000) && (addr <= 0x11ffff))
        ret = ((uint32_t *) riva128->bios_rom.rom)[(addr & riva128->bios_rom.mask) >> 2];
    if (addr >= 0x800000)
...
riva128_mmio_read_w(uint32_t addr, void *priv)
{
    ...
    if ((addr >= 0x110000) && (addr <= 0x11ffff))
        return ((uint16_t *) riva128->bios_rom.rom)[(addr & riva128->bios_rom.mask) >> 1];
...
riva128_init(const device_t *info)
{
    ...
    mem_mapping_add(&riva128->mmio_mapping, 0, 0,
                    riva128_mmio_read, riva128_mmio_read_w, riva128_mmio_read_l,
                    riva128_mmio_write, riva128_mmio_write_w, riva128_mmio_write_l,
                    NULL, MEM_MAPPING_EXTERNAL, riva128);
    mem_mapping_disable(&riva128->mmio_mapping);
    mem_mapping_add(&riva128->linear_mapping, 0, 0,
                    ...
                    NULL, MEM_MAPPING_EXTERNAL, &riva128->svga);
    mem_mapping_disable(&riva128->linear_mapping);
    mem_mapping_add(&riva128->ramin_mapping, 0, 0,
                    riva128_ramin_read, riva128_ramin_read_w, riva128_ramin_read_l,
                    riva128_ramin_write, riva128_ramin_write_w, riva128_ramin_write_l,
                    NULL, MEM_MAPPING_EXTERNAL, riva128);
    mem_mapping_disable(&riva128->ramin_mapping);
    mem_mapping_add(&riva128->linear_mapping2, 0, 0,
                    svga_read_linear, svga_readw_linear, svga_readl_linear,
                    svga_write_linear, svga_writew_linear, svga_writel_linear,
                    NULL, MEM_MAPPING_EXTERNAL, &riva128->svga);
    mem_mapping_disable(&riva128->linear_mapping2);
    mem_mapping_add(&riva128->ramin_mapping2, 0, 0,
                    riva128_ramin_read, riva128_ramin_read_w, riva128_ramin_read_l,
                    riva128_ramin_write, riva128_ramin_write_w, riva128_ramin_write_l,
                    NULL, MEM_MAPPING_EXTERNAL, riva128);
    mem_mapping_disable(&riva128->ramin_mapping2);
    ...
    pci_add_card(PCI_ADD_VIDEO, riva128_pci_read, riva128_pci_write, riva128, &riva128->pci_slot);
