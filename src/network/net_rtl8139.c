...
/* ROM BIOS file paths. */
#define ROM_PATH_RTL8129    "roms/network/rtl8129/rtl8129.rom"
#define ROM_PATH_RTL8139    "roms/network/rtl8239/rtl8139.rom"

/* PCI info. */
#define PCI_VENDID      0x10ec  /* Realtek, Inc */
#define PCI_DEVID_8129  0x8129  /* RTL8129 */
#define PCI_DEVID_8138  0x8138  /* RT8139 (B/C) Cardbus Fast Ethernet Adapter */
#define PCI_DEVID_8139  0x8139  /* RTL8100/8101L/8139 PCI Fast Ethernet Adapter */
#define PCI_REGSIZE     256     /* size of PCI space */

#ifdef ENABLE_RTL81X9_LOG
int rtl81x9_do_log = ENABLE_RTL81X9_LOG;

static void
rtl81x9log(int lvl, const char *fmt, ...)
{
    va_list ap;

    if (rtl81x9_do_log >= lvl) {
        va_start(ap, fmt);
        pclog_ex(fmt, ap);
        va_end(ap);
    }
...
rtl_update_bios(rtl_t *dev)
{
    ...
    if (reg_bios_enable) {
        ...
        rtl81x9log(1, "%s: BIOS now at: %06X\n", dev->name, dev->bios_addr);
    } else {
        rtl81x9log(1, "%s: BIOS disabled\n", dev->name);
        mem_mapping_disable(&dev->bios_rom.mapping);
    }
}

static void
rtl_rom_init(rtl_t *dev, char *s)
{
    uint32_t temp;
    FILE *f;
    ...
    rtl81x9log(1, "%s: BIOS configured at %06lX (size %ld)\n",
               dev->name, dev->bios_addr, dev->bios_size);
}
#endif

static void *
nic_init(UNUSED(const device_t *info))
{
    ...
    rom = (info->local == RTL8129) ? ROM_PATH_RTL8129 : ROM_PATH_RTL8139;
#endif
...
#if 0
    if (info->local == RTL8129) {
        dev->pci_regs[0x02] = (PCI_DEVID_8129&0xff);
        dev->pci_regs[0x03] = (PCI_DEVID_8129>>8);
    } else if (info->local >= RTL8139C) {
        dev->pci_regs[0x02] = (PCI_DEVID_8139&0xff);
        dev->pci_regs[0x03] = (PCI_DEVID_8139>>8);
#endif
    ...
    if (info->local >= RTL8129) {
        ...
        if (info->local == RTL8129) {
            dev->eeprom[0x76] =
            dev->eeprom[0x7A] =
            dev->eeprom[0x7E] = (PCI_DEVID_8129&0xff);
        } else if (info->local >= RTL8139C) {
            dev->eeprom[0x76] =
            dev->eeprom[0x7A] =
            dev->eeprom[0x7E] = (PCI_DEVID_8139&0xff);
        }
        if (info->local == RTL8129) {
            dev->eeprom[0x77] =
            dev->eeprom[0x7B] =
            dev->eeprom[0x7F] = (PCI_DEVID_8129>>8);
        } else if (info->local >= RTL8139C) {
            dev->eeprom[0x77] =
            dev->eeprom[0x7B] =
            dev->eeprom[0x7F] = (PCI_DEVID_8139>>8);
        }
        dev->eeprom[0x78] =
...
// clang-format off
static const device_config_t rtl8139_config[] = {
    {
        .name = "bios",
