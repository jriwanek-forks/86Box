...
typedef struct {
	...
	int irq;
	int dma_channel;
	int64_t cmd_callback;
	int64_t service_req_timer;
	int64_t cip_timer;
} wd33c93_t;
...
{	
	...
	case WD_CMD_SEL:
	{
		...
		if ((wd33c93->dev_id != -1) && scsi_device_present(wd33c93->dev_id, 0))
		{
	...
	case WD_CMD_SEL_XFER:
	{
		...
		if ((wd33c93->dev_id != -1) && scsi_device_present(wd33c93->dev_id, 0))
		{
			scsi_device_t *dev = &scsi_devices[wd33c93->dev_id][wd33c93->lun];
//			scsi_device_t *dev = &scsi_devices[scsi->bus][scsi->cdb_id];

			wd33c93_log("SCSI media type enumerator is %i\n", dev->LunType);
			
			if (wd33c93->regs[WD_COMMAND_PHASE] < 0x45)
			{
				wd33c93->regs[WD_OWN_ID] = scsi_device_cdb_length(wd33c93->dev_id, wd33c93->lun);
				...
				/* do the request */
				scsi_device_command_phase0(wd33c93->dev_id, wd33c93->lun, wd33c93->regs[WD_OWN_ID], &wd33c93->regs[WD_CDB_1]);

				if (SCSIPhase == SCSI_PHASE_DATA_IN)
				{
					dev->CmdBuffer = (uint8_t *) malloc(dev->buffer_length);
				}
				
				wd33c93_log("WD33c93: data length %i\n", dev->buffer_length);
				
				/* set transfer count */
				wd33c93_set_xfer_count(wd33c93, dev->buffer_length);

				switch (SCSIPhase)
				{
					case SCSI_PHASE_DATA_IN:
						wd33c93->read_pending = 1;
						scsi_device_command_phase1(wd33c93->dev_id, wd33c93->lun);
			...
			{
				DMAPageWrite(wd33c93->dma_address,
						(char *)dev->CmdBuffer, dev->buffer_length);
	...
	switch (port & 3)
	{
		...
		case 1:
		{
			...
			else if (wd33c93->latch == WD_DATA)
			{
				...
				wd33c93->dev_id = wd33c93->regs[WD_DESTINATION_ID] & SRCID_MASK;
				wd33c93->lun = (wd33c93->regs[WD_CDB_2] >> 5) & 7;
				scsi_device_t *dev = &SCSIDevices[wd33c93->dev_id][wd33c93->lun];				
				...
				if (dev->buffer_length-- > 0)
				{
					...
					if (dev->buffer_length == 0)
					{
						...
						switch (wd33c93->phase)
						{
							...
							scsi_device_command_phase0(wd33c93->dev_id, wd33c93->lun, wd33c93->regs[WD_OWN_ID], wd33c93->fifo);
							...
							DMAPageRead(wd33c93->dma_address,
								(char *)dev->CmdBuffer, dev->buffer_length);						
	...
	switch(port & 3)
	{
		...
		case 1:
		{
			...
			else if (wd33c93->latch == WD_DATA)
			{
				...
				dev->buffer_length = wd33c93_get_xfer_count(wd33c93);
				
				/* initialize the return value */
				wd33c93->regs[WD_DATA] = 0;
				
				if (dev->buffer_length <= 0 && wd33c93->phase == PHS_MESS_IN)
				{
					/* move to disconnect */
					wd33c93->wdstatus = CSR_DISC;
					wd33c93_complete_cmd(wd33c93);
				}
				else if (dev->buffer_length == 1 && wd33c93->phase == PHS_STATUS)
				{
					/* update the count */
					wd33c93_set_xfer_count(wd33c93, 0);
				...
				else if (dev->buffer_length-- > 0) /* make sure we still have data to send */
				{
					if (wd33c93->read_pending)
					{
						wd33c93->regs[WD_OWN_ID] = scsi_device_cdb_length(wd33c93->dev_id, wd33c93->lun);
...
wd33c93_init(const device_t *info)
{
    ...
    wd33c93 = malloc(sizeof(wd33c93_t));
    memset(wd33c93, 0x00, sizeof(wd33c93_t));
	
	wd33c93->dma_channel = 7;
	wd33c93->irq = 11;
	
	io_sethandler(0x0360, 4,
				wd33c93_read,NULL,NULL, wd33c93_write, NULL,NULL, wd33c93);
