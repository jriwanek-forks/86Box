...
plat_joystick_t      plat_joystick_state[GAMEPORT_MAX][MAX_PLAT_JOYSTICKS];
...
joystick_init(void)
{
    ...
    for (int js = 0; js < joysticks_present; js++) {
        ...
        if (sdl_joy[js]) {
            strncpy(plat_joystick_state[0][js].name, SDL_JoystickNameForIndex(js), 64);
            plat_joystick_state[0][js].nr_axes    = MIN(SDL_JoystickNumAxes(sdl_joy[js]), MAX_JOY_AXES);
            plat_joystick_state[0][js].nr_buttons = MIN(SDL_JoystickNumButtons(sdl_joy[js]), MAX_JOY_BUTTONS);
            plat_joystick_state[0][js].nr_povs    = MIN(SDL_JoystickNumHats(sdl_joy[js]), MAX_JOY_POVS);

            for (int axis_nr = 0; axis_nr < plat_joystick_state[0][js].nr_axes; axis_nr++) {
                snprintf(plat_joystick_state[0][js].axis[axis_nr].name, sizeof(plat_joystick_state[0][js].axis[axis_nr].name), "Axis %i", axis_nr);
                plat_joystick_state[0][js].axis[axis_nr].id = axis_nr;
            }
            for (int button_nr = 0; button_nr < plat_joystick_state[0][js].nr_buttons; button_nr++) {
                snprintf(plat_joystick_state[0][js].button[button_nr].name, sizeof(plat_joystick_state[0][js].button[button_nr].name), "Button %i", button_nr);
                plat_joystick_state[0][js].button[button_nr].id = button_nr;
            }
            for (int pov_nr = 0; pov_nr < plat_joystick_state[0][js].nr_povs; pov_nr++) {
                snprintf(plat_joystick_state[0][js].pov[pov_nr].name, sizeof(plat_joystick_state[0][js].pov[pov_nr].name), "POV %i", pov_nr);
                plat_joystick_state[0][js].pov[pov_nr].id = pov_nr;
            }
        }
    }
}
...
{
    if (mapping & POV_X) {
        switch (plat_joystick_state[0][joystick_nr].p[mapping & 3]) {
            case SDL_HAT_LEFTUP:
            case SDL_HAT_LEFT:
        ...
        return plat_joystick_state[0][joystick_nr].a[plat_joystick_state[0][joystick_nr].axis[mapping].id];
}
...
joystick_process(void)
{
    ...
    for (int js = 0; js < joysticks_present; js++) {
        for (int axis_nr = 0; axis_nr < plat_joystick_state[0][js].nr_axes; axis_nr++)
            plat_joystick_state[0][js].a[axis_nr] = SDL_JoystickGetAxis(sdl_joy[js], axis_nr);

        for (int button_nr = 0; button_nr < plat_joystick_state[0][js].nr_buttons; button_nr++)
            plat_joystick_state[0][js].b[button_nr] = SDL_JoystickGetButton(sdl_joy[js], button_nr);

        for (int pov_nr = 0; pov_nr < plat_joystick_state[0][js].nr_povs; pov_nr++)
            plat_joystick_state[0][js].p[pov_nr] = SDL_JoystickGetHat(sdl_joy[js], pov_nr);
    ...
    for (int js = 0; js < joystick_get_max_joysticks(joystick_type); js++) {
        if (joystick_state[0][js].plat_joystick_nr) {
            ...
            for (int button_nr = 0; button_nr < joystick_get_button_count(joystick_type); button_nr++)
                joystick_state[0][js].button[button_nr] = plat_joystick_state[0][joystick_nr].b[joystick_state[0][js].button_mapping[button_nr]];
