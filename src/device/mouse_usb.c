...
usb_mouse_process_transfer(void* priv, uint8_t* data, uint32_t *len, uint8_t pid_token, uint8_t endpoint, uint8_t underrun_not_allowed)
{
    ...
    if (endpoint == 0) {
        if (pid_token == USB_PID_SETUP) {
            ...
            if (!(setup_packet->bmRequestType & 0x80) && !usb_mouse->device_instance.data_phase)
            {
                usb_mouse->device_instance.data_phase = 1;
                return USB_ERROR_NO_ERROR;
            } else if (!(setup_packet->bmRequestType & 0x80))
                usb_mouse->device_instance.data_phase = 0;

            //pclog("bmRequestType = 0x%X, \n", usb_mouse->device_instance.setup_desc.bmRequestType);
            //pclog("bRequest = 0x%X, \n", usb_mouse->device_instance.setup_desc.bRequest);
            //pclog("wIndex = 0x%X, \n", usb_mouse->device_instance.setup_desc.wIndex);
            //pclog("wLength = 0x%X, \n", usb_mouse->device_instance.setup_desc.wLength);
            //pclog("wValue = 0x%X\n", usb_mouse->device_instance.setup_desc.wValue);
            ...
            /* This is a odd one... */
            if (setup_packet->bmRequestType == (USB_SETUP_TYPE_ENDPOINT | 0x80)
                && setup_packet->bRequest == USB_SETUP_GET_DESCRIPTOR
                && setup_packet->wIndex == 1) {
                switch (setup_packet->wValue >> 8) {
                    case 0x22: {
                        fifo8_push_all(&usb_mouse->device_instance.fifo, qemu_mouse_hid_report_descriptor, sizeof(qemu_mouse_hid_report_descriptor));
                        break;
        ...
        pclog("Polled at %llu\n", plat_timer_read());
        //if ((!mouse_capture && !video_fullscreen) || (!mouse_state_changed()))
        //    return USB_ERROR_NAK;
        usb_mouse_poll(usb_mouse);
        *len = usb_mouse_poll_hid(usb_mouse, data, *len);
        return USB_ERROR_NO_ERROR;
    }
    return USB_ERROR_STALL;
}
...
usb_mouse_poll(void *priv)
{
    ...
    usb_mouse->dx = usb_mouse->dy = 0;
    mouse_subtract_coords(&usb_mouse->dx, &usb_mouse->dy, NULL, NULL, -128, 127, 0, 0);
    mouse_subtract_z(&usb_mouse->dz, -8, 7, 1);
    usb_mouse->buttons_state = mouse_get_buttons_ex();
...
static void usb_mouse_handle_reset(void *priv)
{
    ...
    fifo8_reset(&usb_mouse->device_instance.fifo);
    memset(&usb_mouse->device_instance.setup_desc, 0, sizeof(usb_desc_setup_t));
}
...
usb_mouse_init(const device_t* info)
{
    ...
    usb_mouse->port = usb_attach_device(&usb_mouse->device_instance, USB_BUS_UHCI);
    ...
    usb_mouse_handle_reset(usb_mouse);
    return usb_mouse;
}
