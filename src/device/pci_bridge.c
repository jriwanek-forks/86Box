...
#define AGP_BRIDGE(x)          (((x) >= AGP_BRIDGE_SIS_5XXX) && ((x) != PCI_BRIDGE_INTEL_ICH2))
...
pci_bridge_write(int func, int addr, uint8_t val, void *priv)
{
    ...
    switch (addr) {
        ...
        case 0x07:
            if ((dev->local == AGP_BRIDGE_INTEL_440LX) || (dev->local == AGP_BRIDGE_INTEL_815EP) || (dev->local == AGP_BRIDGE_INTEL_845))
