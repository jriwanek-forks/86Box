    ...
    if (!sw->poll_left) {
        ...
        if (time_since_last > 9900 && time_since_last < 9940) {
            ...
            for (uint8_t js = 0; js < 4; js++) {
                ...
                if (!JOYSTICK_PRESENT(gp_num, js))
                    break;

                if (joystick_state[gp_num][js].axis[1] < -16383)
                    data &= ~1;
                if (joystick_state[gp_num][js].axis[1] > 16383)
                    data &= ~2;
                if (joystick_state[gp_num][js].axis[0] > 16383)
                    data &= ~4;
                if (joystick_state[gp_num][js].axis[0] < -16383)
                    data &= ~8;

                for (uint8_t button_nr = 0; button_nr < 10; button_nr++) {
                    if (joystick_state[gp_num][js].button[button_nr])
                        data &= ~(1 << (button_nr + 4));
                }
