 ...
 */
static uint64_t
cobra_encode_packet(const joystick_t *js_state)
{
    ...
    // Left: data[i] & (1 << 3)
    // Right: data[i] & (1 << 4)
    if (js_state->axis[0] < -16384) decoded_data |= (1 << 3); // Left (bit 3)
    else if (js_state->axis[0] > 16384) decoded_data |= (1 << 4); // Right (bit 4)

    // D-pad Y (Up/Down)
    // Linux driver maps: ABS_Y, -1 to 1.
    // Up: data[i] & (1 << 1)
    // Down: data[i] & (1 << 2)
    if (js_state->axis[1] < -16384) decoded_data |= (1 << 1); // Up (bit 1)
    else if (js_state->axis[1] > 16384) decoded_data |= (1 << 2); // Down (bit 2)
    ...
    if (js_state->button[0]) decoded_data |= (1 << 5);
    // BTN_SELECT (js_state->button[1]) -> bit 6
    if (js_state->button[1]) decoded_data |= (1 << 6);
    // BTN_TL (js_state->button[2]) -> bit 7
    if (js_state->button[2]) decoded_data |= (1 << 7);
    // BTN_TR (js_state->button[3]) -> bit 8
    if (js_state->button[3]) decoded_data |= (1 << 8);
    // BTN_X (js_state->button[4]) -> bit 9
    if (js_state->button[4]) decoded_data |= (1 << 9);
    // BTN_Y (js_state->button[5]) -> bit 10
    if (js_state->button[5]) decoded_data |= (1 << 10);
    // BTN_Z (js_state->button[6]) -> bit 11
    if (js_state->button[6]) decoded_data |= (1 << 11);
    // BTN_A (js_state->button[7]) -> bit 12
    if (js_state->button[7]) decoded_data |= (1 << 12);
    // BTN_B (js_state->button[8]) -> bit 13
    if (js_state->button[8]) decoded_data |= (1 << 13);
    // BTN_C (js_state->button[9]) -> bit 14
    if (js_state->button[9]) decoded_data |= (1 << 14);
    // BTN_TL2 (js_state->button[10]) -> bit 15
    if (js_state->button[10]) decoded_data |= (1 << 15);
    // BTN_TR2 (js_state->button[11]) -> bit 16
    if (js_state->button[11]) decoded_data |= (1 << 16);
 ...
 */
const joystick_if_t joystick_creative_cobra = {
    .name          = "Creative Labs Blaster GamePad Cobra",
    .internal_name = "creative_cobra",
    .init          = cobra_init,         // Custom initialization for proprietary protocol.
    .close         = cobra_close,        // Custom cleanup.
    .read          = cobra_read,         // Custom read for Cobra protocol (pin-specific serial output).
    .write         = cobra_write,        // Custom write to encode state and initiate Cobra polling.
    .read_axis     = cobra_read_axis,    // Axis read for digital gamepad (returns 0).
    .a0_over       = cobra_a0_over,      // Custom A0 overflow handling for protocol timing/handshake.
    .axis_count    = 2,                 // X, Y for D-pad (digital input).
    .button_count  = 12,                // Cobra GamePad typically has 12 buttons.
    .pov_count     = 0,                 // No dedicated POV hat, D-pad is handled by X/Y axes.
    .max_joysticks = 2,                 // Supports 2 daisy-chained devices.
    .axis_names    = { "D-pad X", "D-pad Y" },
    .button_names  = { "Start", "Select", "TL", "TR", "X", "Y", "Z", "A", "B", "C", "TL2", "TR2" }, // Mapped from cobra_btn in Linux driver
    .pov_names     = { NULL }           // No POV hat.
};
