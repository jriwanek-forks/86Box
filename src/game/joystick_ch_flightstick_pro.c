    ...
    if (JOYSTICK_PRESENT(gp_num, 0)) {
        ...
        if (joystick_state[gp_num][0].pov[0] != -1) {
            ...
            if ((joystick_state[gp_num][0].pov[0] > 315) || (joystick_state[gp_num][0].pov[0] < 45))
                ret &= ~0xf0; // 1, 2, 3, 4
            // POV Right
            else if ((joystick_state[gp_num][0].pov[0]) >= 45 && (joystick_state[gp_num][0].pov[0] < 135))
                ret &= ~0xb0; // 1, 2, 4
            // POV Down
            else if ((joystick_state[gp_num][0].pov[0]) >= 135 && (joystick_state[gp_num][0].pov[0] < 225))
                ret &= ~0x70; // 1, 2, 3
            // POV Left
            else if ((joystick_state[gp_num][0].pov[0]) >= 225 && (joystick_state[gp_num][0].pov[0] < 315))
                ret &= ~0x30; // 1, 2
        }
    }
...
ch_virtual_pilot_pro_read(UNUSED(void *priv))
{
    ...
    if (JOYSTICK_PRESENT(gp_num, 0)) {
        ...
        if (joystick_state[gp_num][0].pov[pov_id] != -1) {
            // POV Up
            if ((joystick_state[gp_num][0].pov[pov_id]) > 315 || (joystick_state[gp_num][0].pov[pov_id] < 45))
                ret &= ~0xf0; // 1, 2, 3, 4
            // POV Right
            else if ((joystick_state[gp_num][0].pov[pov_id]) >= 45 && (joystick_state[gp_num][0].pov[pov_id] < 135))
                ret &= ~0xb0; // 1, 2, 4
            // POV Down
            else if ((joystick_state[gp_num][0].pov[pov_id]) >= 135 && (joystick_state[gp_num][0].pov[pov_id] < 225))
                ret &= ~0x70; // 1, 2, 3
            // POV Left
            else if ((joystick_state[gp_num][0].pov[pov_id]) >= 225 && (joystick_state[gp_num][0].pov[pov_id] < 315))
                ret &= ~0x30; // 1, 2
        }
