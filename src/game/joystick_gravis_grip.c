 ...
 *
 * This implementation is based on insights from the Linux kernel's `grip.c`
 * driver, which provides a more accurate understanding of the pin usage
 * and data transfer mechanism for GrIP devices, especially the Gravis GamePad Pro (GPP mode).
 *
 * Authors: Jasmine Iwanek, <jriwanek@gmail.com> (initial structure based on existing 86Box files)
 * Original Gravis GrIP protocol reverse-engineered by others (details from Linux `grip.c` driver).
 *
...
#include <math.h>         // For M_PI, used in POV angle calculations.
...
typedef struct grav_data {
    ...
    uint8_t    packet_buffer[4 * 3]; /**< Buffer for 4 joysticks * 3 bytes/joystick = 12 bytes total (96 bits). */
                                     // Each gamepad sends a 24-bit packet for GPP mode.
...
grav_read_gpp(void *priv)
{
    ...
    if (timer_is_enabled(&grav->poll_timer)) {
        ...
        if (joystick_state[0][0].button[0]) ret &= ~0x10; // B1 (Pin 2)
        if (joystick_state[0][0].button[1]) ret &= ~0x20; // B2 (Pin 7)
        if (joystick_state[0][0].button[2]) ret &= ~0x40; // B3 (Pin 10)
        if (joystick_state[0][0].button[3]) ret &= ~0x80; // B4 (Pin 14)
    }
...
grav_xt_bd_dc_read(void *priv)
{
    ...
    // ADDED: Minimal serial response for XT/BD/DC.
    // This is highly simplified and not protocol accurate.
    if (timer_is_enabled(&grav->poll_timer)) {
...
grav_xt_bd_dc_write(void *priv)
{
    ...
    // ADDED: Minimal write response for XT/BD/DC.
    // This is highly simplified and not protocol accurate.
    if (!timer_is_enabled(&grav->poll_timer)) {
        grav->poll_clock = 0;
...
{
    return 0; // Axes centered/inactive (protocol is digital).
}
...
grav_a0_over(void *priv)
{
    ...
    if (grav->id_request_pending) {
        ...
        timer_disable(&grav->handshake_timer); // Complete handshake or timeout.
...
const joystick_if_t joystick_gravis_gamepad_pro = {
    ...
    .init          = grav_init,         // Custom initialization for proprietary protocol.
    .close         = grav_close,        // Custom cleanup.
    .read          = grav_read_gpp,     // Custom read for GPP protocol (pin-specific serial output).
    .write         = grav_write_gpp,    // Custom write to encode state and initiate GPP polling.
    .read_axis     = grav_read_axis,    // Axis read for digital gamepad (returns 0).
    .a0_over       = grav_a0_over,      // Custom A0 overflow handling for protocol timing/handshake.
    .axis_count    = 2,                 // X, Y for D-pad (digital input).
    .button_count  = 10,                // Gravis GamePad Pro commonly has 10 buttons.
    .pov_count     = 1,                 // Has a digital POV hat (D-pad).
    .max_joysticks = 2,                 // Based on `grip.c`'s `(i << 1) + 4` shift, supporting 2 devices on 4 button lines.
                                        // While GrIP could daisy-chain more, 4 button lines typically restrict it to 2 distinct devices for this type of protocol.
    .axis_names    = { "D-pad X", "D-pad Y" },
    .button_names  = { "Select", "Start", "R2", "Blue", "L2", "Green", "Yellow", "Red", "L1", "R1" }, // Mapped from image functions
    .pov_names     = { "D-Pad" }
};
 ...
 *
 * DB-15 Pin Connections:
 * Uses standard gameport pins for serial communication, but with the `grip_xt_read_packet` protocol.
 */
const joystick_if_t joystick_gravis_blackhawk_digital = {
    .name          = "Gravis Blackhawk Digital",
    .internal_name = "gravis_blackhawk_digital",
    .init          = grav_init,          // Reuses common init.
    .close         = grav_close,         // Reuses common close.
    .read          = grav_xt_bd_dc_read, // Placeholder for XT/BD/DC protocol read.
    .write         = grav_xt_bd_dc_write, // Placeholder for XT/BD/DC protocol write.
    .read_axis     = grav_xt_bd_dc_read_axis, // Placeholder for XT/BD/DC axis read.
    .a0_over       = grav_xt_bd_dc_a0_over,       // Placeholder for A0 overflow handling.
    .axis_count    = 3,                  // X, Y, Throttle (from grip_abs_bd in grip.c).
    .button_count  = 5,                  // Based on grip_btn_bd in grip.c.
    .pov_count     = 1,                  // Has a POV hat (from grip_abs_bd in grip.c).
    .max_joysticks = 1,                  // Typically a single device.
    .axis_names    = { "X axis", "Y axis", "Throttle" },
    .button_names  = { "Thumb", "Thumb 2", "Trigger", "Top", "Base" }, // Based on grip_btn_bd in grip.c.
    .pov_names     = { "POV" }
};
 ...
 * Uses standard gameport pins for serial communication, but with the `grip_xt_read_packet` protocol.
 */
const joystick_if_t joystick_gravis_xterminator_digital = {
    ...
    .read_axis     = grav_xt_bd_dc_read_axis, // Placeholder for XT/BD/DC axis read.
    .a0_over       = grav_xt_bd_dc_a0_over,       // Placeholder for A0 overflow handling.
    .axis_count    = 5,                  // X, Y, Brake, Gas, Throttle (from grip_abs_xt in grip.c).
    .button_count  = 11,                 // Based on grip_btn_xt in grip.c.
    .pov_count     = 2,                  // Has two POV hats (from grip_abs_xt in grip.c).
    .max_joysticks = 1,                  // Typically a single device.
    .axis_names    = { "X axis", "Y axis", "Brake", "Gas", "Throttle" },
    .button_names  = { "Trigger", "Thumb", "A", "B", "C", "X", "Y", "Z", "Select", "Start", "Mode" }, // Based on grip_btn_xt in grip.c.
 ...
 * Uses standard gameport pins for serial communication, but with the `grip_xt_read_packet` protocol.
 */
const joystick_if_t joystick_gravis_xterminator_dualcontrol = {
    ...
    .read_axis     = grav_xt_bd_dc_read_axis, // Placeholder for XT/BD/DC axis read.
    .a0_over       = grav_xt_bd_dc_a0_over,       // Placeholder for A0 overflow handling.
    .axis_count    = 5,                  // X, Y, RX, RY, Throttle (from grip_abs_dc in grip.c).
    .button_count  = 9,                  // Based on grip_btn_dc in grip.c.
    .pov_count     = 1,                  // Has a POV hat (from grip_abs_dc in grip.c).
    .max_joysticks = 1,                  // Typically a single device.
    .axis_names    = { "X axis", "Y axis", "RX axis", "RY axis", "Throttle" },
    .button_names  = { "Trigger", "Thumb", "Top", "Top 2", "Base", "Base 2", "Base 3", "Base 4", "Base 5" }, // Based on grip_btn_dc in grip.c.
    .pov_names     = { "POV" }
};
