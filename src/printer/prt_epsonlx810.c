...
typedef struct lpt_epsonlx810_t {
...
void printChar(lpt_epsonlx810_t *printer);

// Hard Reset (like switching printer off and on)
void resetPrinterHard(lpt_epsonlx810_t *printer);

// Set Autofeed value
void setAutofeed(bool feed, lpt_epsonlx810_t *printer);

// Get Autofeed value
bool getAutofeed(lpt_epsonlx810_t *printer);

// True if printer is unable to process more data right now (do not use printChar)
bool isBusy(lpt_epsonlx810_t *printer);

// True if the last sent character was received
bool ack(lpt_epsonlx810_t *printer);

// Manual formfeed
void formFeed(lpt_epsonlx810_t *printer);

// Returns true if the current page is blank
bool isBlank(lpt_epsonlx810_t *printer);

// Checks if given char belongs to a command and process it. If false, the character
// should be printed
bool processCommandChar(uint8_t ch, lpt_epsonlx810_t *printer);

// Resets the printer to the factory settings
void resetPrinter(lpt_epsonlx810_t *printer, int software_command);

// Reload font. Must be called after changing dpi, style or cpi
void updateFont(lpt_epsonlx810_t *printer);

// Clears page. If save is true, saves the current page to a bitmap
void newPage(bool save, lpt_epsonlx810_t *printer);
...
void blitGlyph(FT_Bitmap bitmap, uint16_t destx, uint16_t desty, bool add, lpt_epsonlx810_t *printer);

// Draws an anti-aliased line from (fromx, y) to (tox, y).
void drawLine(uint16_t fromx, uint16_t tox, uint16_t y, lpt_epsonlx810_t *printer);

// Setup the bitGraph structure
void setupBitImage(uint8_t dens, uint16_t numCols, lpt_epsonlx810_t *printer);

// Process a character that is part of bit image. Must be called iff bitGraph.remBytes > 0.
void printBitGraph(uint8_t ch, lpt_epsonlx810_t *printer);

// Copies the codepage mapping from the constant array to CurMap, along with international character sets for character < 0x80
void updateCharset(lpt_epsonlx810_t *printer);

// Output current page
void outputPage(lpt_epsonlx810_t *printer);

// Prints out a byte using ASCII85 encoding (only outputs something every four bytes). When b>255, closes the ASCII85 string
void fprintASCII85(FILE *f, uint16_t b, lpt_epsonlx810_t *printer);

// Closes a multipage document
void finishMultipage(lpt_epsonlx810_t *printer);

// Returns value of the num-th pixel (couting left-right, top-down) in a safe way
uint8_t getPixel(uint32_t num, lpt_epsonlx810_t *printer);
...
#ifdef ENABLE_EPSONLX810_LOG
int epsonlx810_do_log = ENABLE_AUDIOPCI_LOG;

static void
epsonlx810_log(const char *fmt, ...)
{
    va_list ap;

    if (epsonlx810_do_log) {
        va_start(ap, fmt);
        pclog_ex(fmt, ap);
        va_end(ap);
    }
...
#else
#    define epsonlx810_log(fmt, ...)
#endif

void
resetPrinterHard(lpt_epsonlx810_t *printer)
{
    printer->charRead = false;
    resetPrinter(printer, false);
}

// TODO: see exactly what is reset with ESC @
void
resetPrinter(lpt_epsonlx810_t *printer, int software_command)
{
    if (!software_command) {
...
void
updateCharset(lpt_epsonlx810_t *printer)
{
    ...
    switch (cp) {
        ...
        case 865:
            ...
            break;
        default:
            epsonlx810_log("Unsupported codepage %i. Using CP437 instead.\n", cp);
            mapToUse = (uint16_t *) &cp437Map;
    }
...
void
updateFont(lpt_epsonlx810_t *printer)
{
    ...
    if (printer->printQuality == QUALITY_NLQ) {
        ...
        if (FT_New_Face(printer->FTlib, fullpath, 0, &printer->curFont)) {
            epsonlx810_log("Unable to load font %s, trying default\n", fullpath);
            strlcpy(fullpath, printer->fontpath, MAX_PATH_STRING);
            strlcat(fullpath, "fonts/", MAX_PATH_STRING);
            strlcat(fullpath, fontName_Base, MAX_PATH_STRING);
            strlcat(fullpath, fontName_cpi, MAX_PATH_STRING);
...
bool
processCommandChar(uint8_t ch, lpt_epsonlx810_t *printer)
{
    if (printer->ESCSeen) {
        printer->ESCCmd   = ch;
        printer->ESCSeen  = false;
        ...
        switch (printer->ESCCmd) {
            ...
            case 0x43: // Set page length in lines (ESC C)
                if (printer->params[0] != 0)
                    printer->pageHeight = printer->bottomMargin = (double) printer->params[0] * printer->lineSpacing; // TODO: is this right? Also, we have ALREADY allocated the SDL
            ...
            case 0x49: // Enable printing of control codes (ESC I n)
                ...
                epsonlx810_log("EPSON ESC I: printing of control codes not implemented on the LX-810!\n");
                break;
            case 0x4a:                                                         // Advance print position vertically (ESC J n)
            ...
            case 0x70: // Turn proportional mode on/off (ESC p)
                ...
                epsonlx810_log("EPSON ESC p: proportional mode not supported by the LX-810!\n");
                break;
            case 0x73: // Select low-speed mode (ESC s)
                // Ignore
                break;
            ...
            case 0x100: // Set page length in inches (ESC C NUL)
                ...
                break;
            default:
                epsonlx810_log("PRINTER: Skipped unsupported command ESC %c (%02X)\n", printer->ESCCmd, printer->ESCCmd);
        }
...
void
newPage(bool save, lpt_epsonlx810_t *printer)
{
    if (save)
        outputPage(printer);
...
void
printChar(lpt_epsonlx810_t *printer)
{
...
void
blitGlyph(FT_Bitmap bitmap, uint16_t destx, uint16_t desty, bool add, lpt_epsonlx810_t *printer)
{
...
void
drawLine(uint16_t fromx, uint16_t tox, uint16_t y, lpt_epsonlx810_t *printer)
{
    SDL_LockSurface(printer->page);
...
void
setAutofeed(bool feed, lpt_epsonlx810_t *printer)
{
    printer->autoFeed = feed;
}

bool
getAutofeed(lpt_epsonlx810_t *printer)
{
    return printer->autoFeed;
}

bool
isBusy(lpt_epsonlx810_t *printer)
{
    // We're never busy
    return false;
}
...
void
setupBitImage(uint8_t dens, uint16_t numCols, lpt_epsonlx810_t *printer)
{
    switch (dens) {
...
void
printBitGraph(uint8_t ch, lpt_epsonlx810_t *printer)
{
...
void
formFeed(lpt_epsonlx810_t *printer)
{
...
static void
findNextName(const char *front, const char *ext, char *filename, size_t filename_size, const char *base_path)
{
    char     buffer[10];
    uint16_t num  = 1;
    FILE    *test = NULL;
...
void
outputPage(lpt_epsonlx810_t *printer)
{
    ...
    if (printer->output_type == OUTPUT_TYPE_FORWARD_TO_REAL_PRINTER) {
        ...
        while (pix < numpix) {
            ...
            if ((pix < numpix - 2) && (getPixel(pix, printer) == getPixel(pix + 1, printer)) && (getPixel(pix, printer) == getPixel(pix + 2, printer))) {
                ...
                uint8_t diffCount = 1;
                while (diffCount < 128 && diffCount + pix < numpix && ((diffCount + pix < numpix - 2) || (getPixel(pix + diffCount, printer) != getPixel(pix + diffCount + 1, printer)) || (getPixel(pix + diffCount, printer) != getPixel(pix + diffCount + 2, printer))))
                    diffCount++;
...
void
fprintASCII85(FILE *f, uint16_t b, lpt_epsonlx810_t *printer)
{
    if (b != 256) {
        if (b < 256)
...
void
finishMultipage(lpt_epsonlx810_t *printer)
{
    if (printer->outputHandle != NULL) {
...
bool
isBlank(lpt_epsonlx810_t *printer)
{
    bool blank = true;
...
uint8_t
getPixel(uint32_t num, lpt_epsonlx810_t *printer)
{
    // Respect the pitch
...
static void
epsonlx810_write_data(uint8_t val, void *priv)
{
    lpt_epsonlx810_t *lpt_epsonlx810 = (lpt_epsonlx810_t *) priv;
#ifdef PRINTER_DEBUG
    epsonlx810_log("lx-810 write data %02X\n", val);
#endif
    lpt_epsonlx810->last_data = val;
}

static void
epsonlx810_write_ctrl(uint8_t val, void *priv)
{
    lpt_epsonlx810_t *lpt_epsonlx810 = (lpt_epsonlx810_t *) priv;
#ifdef PRINTER_DEBUG
    epsonlx810_log("lx-810 write control %02X\n", val);
#endif

    setAutofeed((val & 0x02) != 0, lpt_epsonlx810);
    if ((val & 0x01) && (!(lpt_epsonlx810->controlreg & 0x01)))
        printChar(lpt_epsonlx810);
    if ((val & 0x04) && (!(lpt_epsonlx810->controlreg & 0x04)))
        resetPrinterHard(lpt_epsonlx810);

    lpt_epsonlx810->controlreg = val;
}

static uint8_t
epsonlx810_read_status(void *priv)
{
    lpt_epsonlx810_t *lpt_epsonlx810 = (lpt_epsonlx810_t *) priv;
    uint8_t           temp;
    ...
    // if (lpt_epsonlx810->controlreg & 0x08 == 0)
    //     temp |= 0x10;

    if (!isBusy(lpt_epsonlx810))
        temp |= 0x80;

    if (!ack(lpt_epsonlx810))
        temp |= 0x40;
#ifdef PRINTER_DEBUG
    epsonlx810_log("lx-810 read status %02X\n", temp);
#endif
    return temp;
}

static uint8_t
epsonlx810_read_ctrl(void *priv)
{
    lpt_epsonlx810_t *lpt_epsonlx810 = (lpt_epsonlx810_t *) priv;
    uint8_t           temp           = 0xe0 | (getAutofeed(lpt_epsonlx810) ? 0x02 : 0x00) | (lpt_epsonlx810->controlreg & 0xfd);
#ifdef PRINTER_DEBUG
    epsonlx810_log("lx-810 read control %02X\n", temp);
#endif
...
static void *
epsonlx810_init(void* lpt)
{
    // TODO: 86Box: lpt never assigned to device
    lpt_epsonlx810_t *lpt_epsonlx810 = (lpt_epsonlx810_t *) malloc(sizeof(lpt_epsonlx810_t));
    memset(lpt_epsonlx810, 0, sizeof(lpt_epsonlx810_t));

    lpt_epsonlx810->controlreg = 0x04;
    ...
    char s[512] = { 0 };
    strlcpy(lpt_epsonlx810->fontpath, printer_path, MAX_PATH_STRING - 2);
    // Ensure the path ends with a separator.
    path_slash(lpt_epsonlx810->fontpath);

    strlcpy(lpt_epsonlx810->outputpath, printer_path, MAX_PATH_STRING - 2);
    // Ensure the path ends with a separator.
    path_slash(lpt_epsonlx810->outputpath);

    if (FT_Init_FreeType(&lpt_epsonlx810->FTlib)) {
        fatal("PRINTER: Unable to init Freetype2. Printing disabled\n");
        lpt_epsonlx810->page = NULL;
    } else {
        lpt_epsonlx810->dpi             = dpi;
        lpt_epsonlx810->output_type     = output_type;
        lpt_epsonlx810->multipageOutput = multipage;

        lpt_epsonlx810->defaultPageWidth  = (double) width / (double) 10;
        lpt_epsonlx810->defaultPageHeight = (double) height / (double) 10;

        lpt_epsonlx810->defaultCPI            = cpi;
        lpt_epsonlx810->defaultStyle          = style;
        lpt_epsonlx810->defaultQuality        = quality;
        lpt_epsonlx810->defaultNLQtypeFace    = font;
        lpt_epsonlx810->defaultCharTable      = chartable;
        lpt_epsonlx810->defaultI18NCharset    = i18ncharset;
        lpt_epsonlx810->defaultCodePage       = codepage;
        lpt_epsonlx810->emulatePinsForBitmaps = emulatepins;

        // Create page
        lpt_epsonlx810->page = SDL_CreateRGBSurface(
            0, (unsigned int) (lpt_epsonlx810->defaultPageWidth * lpt_epsonlx810->dpi),
            (unsigned int) (lpt_epsonlx810->defaultPageHeight * lpt_epsonlx810->dpi), 8, 0, 0, 0, 0);

        // Set a grey palette
        SDL_Palette *palette = lpt_epsonlx810->page->format->palette;

        for (uint16_t i = 0; i < 256; i++)
            palette->colors[i].r = palette->colors[i].g = palette->colors[i].b = 255 - i;

        lpt_epsonlx810->curFont      = NULL;
        lpt_epsonlx810->autoFeed     = false;
        lpt_epsonlx810->outputHandle = NULL;

        resetPrinterHard(lpt_epsonlx810);

        if (lpt_epsonlx810->output_type == OUTPUT_TYPE_FORWARD_TO_REAL_PRINTER) {
#if defined(WIN32)
            ...
            lpt_epsonlx810->printerDC = pd.hDC;
#endif
        }
        epsonlx810_log("PRINTER: Enabled\n");
    }

    // TODO: MAPPER_AddHandler(FormFeed,MK_f2,MMOD1,"ejectpage","formfeed");

    return lpt_epsonlx810;
}

static void
epsonlx810_close(void *priv)
{
    lpt_epsonlx810_t *lpt_epsonlx810 = (lpt_epsonlx810_t *) priv;

    formFeed(lpt_epsonlx810); // eject any pending page
    if (lpt_epsonlx810->page != NULL) {
        SDL_FreeSurface(lpt_epsonlx810->page);
        lpt_epsonlx810->page = NULL;
        FT_Done_FreeType(lpt_epsonlx810->FTlib);
    }
#if defined(WIN32)
    DeleteDC(lpt_epsonlx810->printerDC);
#endif

    free(lpt_epsonlx810);
}

static device_config_t epsonlx810_config[] = {
    // clang-format off
    {
        .name        = "dpi",
    ...
    },
    {
        ...
        .type        = CONFIG_SELECTION,
        .selection   = {
            { .description = "10 CPI", .value = 10 },
            { .description = "12 CPI", .value = 12 },
            {.description = ""                     }},
        .default_int = 10},
#if 0
    // TODO
    {
        ...
        .type        = CONFIG_SELECTION,
        .selection   = {
            { .description = "Not Slashed", .value = 0 },
            { .description = "Slashed",     .value = 1 },
            { .description = ""                        }
        },
        .default_int = 0
    },
    ...
    // TODO
    {
        ...
        .type        = CONFIG_SELECTION,
        .selection   = {
            { .description = "Normal",     .value = 0 },
            // TODO: different font, only works at 10 CPI! Reverts do "Draft" otherwise (what about styles?)
...
#endif
    {
        ...
        .selection   = {
            ...
            { .description = "CP437 + Latin America (Only available as a printer command on real hardware)", .value = 13 },
    ...
    if it goes back to these values after a software/command reset. */
    {
...
const lpt_device_t lpt_prt_epsonlx810_device = {
    .name          = "Epson LX-810 Printer",
    .internal_name = "epsonlx810",
    .init          = epsonlx810_init,
    .close         = epsonlx810_close,
    .write_data    = epsonlx810_write_data,
    .write_ctrl    = epsonlx810_write_ctrl,
    .read_data     = NULL,
    .read_status   = epsonlx810_read_status,
    .read_ctrl     = epsonlx810_read_ctrl,
    .config        = epsonlx810_config
};
